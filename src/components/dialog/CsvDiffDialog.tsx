    ...
    if (entry.status === "equal") {
      ...
      return (
        ...
        >
          ...
          >
            ≈ {entry.count} {t.csvDiffEqualRows}
          </td>
          <td
              ...
              <div className="flex flex-1 items-center justify-center gap-3 text-xs">
                <div className="flex items-center gap-3">
                  <span className="flex items-center gap-1 text-muted-foreground">
                    ...
                    {t.csvDiffAdded} {result.added_count}
                  </span>
                  <span className="flex items-center gap-1 text-muted-foreground">
                    <span className="w-2.5 h-2.5 rounded-sm bg-red-500/60" />
                    {t.csvDiffRemoved} {result.removed_count}
                  </span>
                  <span className="flex items-center gap-1 text-muted-foreground">
                    <span className="w-2.5 h-2.5 rounded-sm bg-yellow-500/60" />
                    {t.csvDiffModified} {result.modified_count}
                  </span>
                  <span className="flex items-center gap-1 text-muted-foreground">
                    <span className="w-2.5 h-2.5 rounded-sm bg-border" />
                    {t.csvDiffEqual} {result.equal_count}
                  </span>
                </div>
                <span className="text-xs text-muted-foreground/70 shrink-0">
                  {result.headers_left.length} {t.csvDiffColsLeft} /{" "}
                  {result.headers_right.length} {t.csvDiffColsRight}
                </span>
              </div>
            )}
            <Button
          ...
          {result ? (
              ...
              {result.entries.length > PAGE_SIZE && (
                <div className="flex items-center gap-2 pb-2 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {t.csvDiffPage} {page + 1} / {totalPages}
                  </span>
                  <Button
                    variant="secondary"
                    size="sm"
            ...
            <div className="h-full flex items-center justify-center">
              <p className="text-sm text-muted-foreground">
                {isComparing ? t.csvDiffComparing : t.csvDiffNoResult}
              </p>
            </div>
          )}
        </ScrollArea>
