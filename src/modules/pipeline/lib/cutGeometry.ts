...
export function getCutIntersectionPoints(
  ...
  for (const edge of edges) {
    ...
    const t =
      ((edge.x1 - lineStart.x) * ey - (edge.y1 - lineStart.y) * ex) / denom;
    const u =
      ((edge.x1 - lineStart.x) * dy - (edge.y1 - lineStart.y) * dx) / denom;
  ...
  for (const p of intersections) {
    if (
      !unique.some((u) => Math.abs(u.x - p.x) < 2 && Math.abs(u.y - p.y) < 2)
    ) {
      unique.push(p);
    }
  }
...
// 计算坠落方向(重力 + 切割方向混合)
export function calculateFallVector(cutPath: { x: number; y: number }[]): {
  dx: number;
  dy: number;
  rotation: number;
} {
