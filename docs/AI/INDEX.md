...
┌────────────────────▼────────────────────────────────┐
│               Backend (Rust / Tauri)                │
│  src-tauri/src/lib.rs (954 行,全部后端逻辑)           │
│  17 个 Tauri 命令处理器                               │
...
所有后端逻辑集中在 `lib.rs` 一个文件中(954 行),无内部模块拆分。

| 文件 | 行数 | 职责 |
|------|------|------|
| `main.rs` | 6 | 二进制入口,调用 `easy_csv::run()`。Windows release 模式下隐藏控制台窗口 |
| `lib.rs` | 954 | **全部后端逻辑**,详见下方分解 |
...
| Tauri 命令 | 57-130+ | 17 个 IPC 命令注册 |
| 管道执行 | ~89-516 | `execute_xan_pipeline` — 核心函数,构建 CLI 参数、多进程管道、进程管理 |
| 数据分析 | ~92-130 | `profile_csv` — 调用 `xan stats` 进行数据概况分析 |
| 缓存管理 | ~744-860 | `load_profile_cache` / `save_profile_cache` — 带 LRU 淘汰的 JSON 缓存 |
| 配置管理 | ~601-740 | 配置文件读写、资源目录、xan.exe 解压与查找 |
| 入口函数 | ~875-954 | `run()` — 注册插件/命令,系统托盘,窗口事件处理 |
...
| `load_profile_cache` / `save_profile_cache` | 数据概况缓存(基于文件 mtime,LRU 淘汰,上限50条) |
...
| `App.tsx` | **根组件**(1005行),管理所有状态：标签页、管道、撤销/重做、日志、配置、历史记录、更新检查、拖拽打开、数据概况 |
...
| `i18n/translations.ts` | 中英文翻译字符串(~70个 key) |
...
| 修改系统托盘/窗口行为 | `src-tauri/src/lib.rs` 中的 `setup()` 和 `on_window_event` |
| 修改数据概况功能 | `src/components/panel/DataProfilePanel.tsx` + `lib.rs` 中的 `profile_csv`/缓存函数 |
